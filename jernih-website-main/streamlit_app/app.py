    ...
    if prompt := st.chat_input(t("Tanyakan sesuatu...", "Ask something...")):
        ...
        with st.chat_message("assistant"):
            with st.spinner(t("Mencari informasi...", "Searching for information...")):
                        # Cek apakah ini pertanyaan tanggal/waktu
                from src.models import RAGResult
                from datetime import datetime
                prompt_lower = prompt.lower()
                is_date_question = any(k in prompt_lower for k in ["tanggal", "hari ini", "besok", "hari apa", "jam berapa"])
        
                if is_date_question:
            # Jika pertanyaan tanggal, jangan panggil RAG agar tidak muncul file txt
                    rag = RAGResult(context="", sources=[])
                else:
            # Jika bukan pertanyaan tanggal, baru lakukan pencarian file
