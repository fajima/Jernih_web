...
Current year is 2026. Answer using DATA ABOVE, not your old knowledge."""
    ...
    def _answer_direct(self, query: str, lang: str) -> str | None:
        ...
        if lang == "id":
            import re
            # Bersihkan tanda baca (?, !, .) agar "tanggal brp?" cocok dengan kata kunci
            q_clean = re.sub(r'[^\w\s]', '', q)
            
            if q_clean in ("hari ini hari apa", "hari apa sekarang", "sekarang hari apa", "tanggal berapa hari ini", "hari ini tanggal berapa", "hari ini tanggal brp", "tanggal hari ini", "sekarang tanggal berapa"):
                return f"Hari ini {days_id[today_idx]}, {now.day} {months_id[now.month-1]} {now.year}."
            if q_clean in ("besok hari apa", "hari apa besok", "tanggal berapa besok", "besok tanggal berapa", "besok tanggal brp", "besok hari tanggal berapa"):
                from datetime import timedelta
                tom = now + timedelta(days=1)
                return f"Besok {days_id[tom.weekday()]}, {tom.day} {months_id[tom.month-1]} {tom.year}."
            if q_clean in ("kemarin hari apa", "hari apa kemarin"):
                from datetime import timedelta
                yes = now - timedelta(days=1)
                return f"Kemarin {days_id[yes.weekday()]}, {yes.day} {months_id[yes.month-1]} {yes.year}."
            if "jam berapa" in q_clean:
    ...
    def ask(self, query: str, rag_result: RAGResult, lang: str = "id", history: list | None = None) -> CopilotResponse:
        ...
        if history and len(history) > 0:
            # Ambil 4 pesan terakhir (2 pasang tanya-jawab sebelumnya)
            recent_history = history[-4:] if len(history) > 4 else history
            history_text = "\n--- Percakapan sebelumnya ---\n"
            for msg in recent_history:
                role = "User" if msg.get("role") == "user" else "JERNIH"
                content = msg.get("content", "")
                # Potong jika terlalu panjang agar tidak melebihi token limit
                content = content[:600] if len(content) > 600 else content
                history_text += f"{role}: {content}\n"
            history_text += "--- Akhir percakapan sebelumnya ---\n\n"

        if lang == "id":
            user_msg = f"{history_text}{context_block}\n\nPertanyaan saat ini: {query}\n\n{sources_block}"
        else:
